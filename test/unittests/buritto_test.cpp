    ...
    auto pushThread = std::thread([&] {
        ...
        for (uint64_t i = 0; i < NUMBER_OF_PUSHES; i++) {
            ...
            if (buritto.push(pushCounter, out) == true) {
                if (out != BuRiTTO_InvalidValue) {
                    std::cout << "Failure: pushThread BuRiTTO should not return data!" << std::endl;
                    break;
                }
            } else {
                if (out == BuRiTTO_InvalidValue) {
                    std::cout << "Failure: pushThread BuRiTTO should return data!" << std::endl;
                    break;
                }
                overrunCounter++;
    ...
    auto popThread = std::thread([&] {
        ...
        while (!pushThreadFinished.load(std::memory_order_relaxed) || !buritto.empty()) {
            ...
            if (buritto.pop(out) == true) {
                if (out == BuRiTTO_InvalidValue) {
                    std::cout << "Failure: popThread BuRiTTO should return data!" << std::endl;
                    break;
                }
                popCounter++;
                ...
                if (out != BuRiTTO_InvalidValue) {
                    std::cout << "Failure: popThread BuRiTTO should not return data!" << std::endl;
                    break;
                }
            }
        }
...
TEST_CASE("BuRiTTO - Benchmark", "[!benchmark]") {
    ...
    const DataType expectedSum {[&]() -> DataType {
        DataType sum {0};
